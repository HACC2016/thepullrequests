...
<?php if ($privacy_policy and trim($privacy_policy) != ''){ ?>  
  <h2><?php _e('Summary', 'terms_of_use') ?></h2>
...
<?php if ($show_buttons){ ?>
    ...
    <form id="post" method="post" action="" name="post">
    	...
    	<?php if ($tou_settings->initials){?>
    	    <?php _e('Sign Here', 'terms_of_use') ?> <input type="text" name="initials" size="50" value="<?php echo (isset($_POST['initials'])) ? $_POST['initials'] : ''; ?>" />
    	<?php } ?>
    	<p class="submit">

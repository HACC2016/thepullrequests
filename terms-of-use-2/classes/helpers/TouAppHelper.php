...
class TouAppHelper{
    ...
    static function get_param($param, $default=''){
   ...
   static  function get_user_meta($user_ID, $meta_key){
   ...
   static  function update_user_meta($user_ID, $meta_key, $value){
   ...
   static  function save_meta($user_ID, $initials='', $date=false){
        if(!$user_ID)
            return;
  ...
  static   function clear_agreement($user_ID){
   ...
   static  function get_include_contents($filename, $atts=array()) {
        extract($atts);
   ...
   static  function forms_checkboxes( $field_name, $field_value=array(), $cols=1 ){
    ...
    static function insert_checkbox($errors='', $location='register'){
        global $tou_settings;
   ...
   static  function checked($values, $current){
        if(in_array($current, (array)$values))
            echo ' checked="checked"';
    }
    
   static  function show_date($date, $initials='', $html=true){
        global $tou_settings;
